...
function VapourIntroInner({
  ...
  const runExitSlide = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      ...
      clearExitSafety()
      exitSafetyRef.current = window.setTimeout(
        finishIntro,
        INTRO_EXIT_MS + 250,
      )
  ...
  return (
    ...
    >
      <div
        ref={motionLayerRef}
        className="fifth-intro-panel pointer-events-none absolute inset-0"
      >
        <div
      ...
      switch (animationState) {
        ...
        case "vaporizing": {
          ...
          const overlapStart = Math.min(
            90,
            Math.max(8, animation.overlapStartPercent ?? 38),
          )
          ...
          if (
            ...
            const { particles: inc, textBoundaries: tb } =
              sampleWordParticlesInto(
                wrapperSize.width,
                wrapperSize.height,
                globalDpr,
        ...
        case "scattering": {
          ...
          if (
            scatterTimeRef.current >= scatterSecs &&
            !sequenceDoneRef.current
          ) {
            sequenceDoneRef.current = true
            setAnimationState("complete")
