...
export function FullScreenCalendar({
  ...
  const openEditEventRef = React.useRef<
    ((event: CalendarEventItem) => void) | null
  >(null)
  ...
  const openEditEvent = React.useCallback(
    (event: CalendarEventItem) => {
      ...
      setEventDate(
        startDate
          ? toLocalDateValue(startDate)
          : toLocalDateValue(selectedDate),
      )
  ...
  function validateStep(step: number) {
    if (step === 0) {
      ...
      if (eventIsMeeting && !eventMeetingLink.trim())
    ...
    if (step === 1) {
      ...
      const startParsed = safeParseDateTime(
        buildLocalDateTime(eventDate, eventStartTime),
      )
      ...
      if (!startParsed)
        return "Informe uma data e um horário de início válidos."
  ...
  async function submitEvent() {
    ...
    if (error) {
      setFormError(error)
      setCurrentStep(
        error.includes("título") || error.includes("workspace") ? 0 : 1,
      )
      return
    }
    ...
    try {
      ...
      const endAt = eventEndTime
        ? `${buildLocalDateTime(eventDate, eventEndTime)}:00`
        : undefined
      ...
      setFormError(
        error instanceof Error ? error.message : "Falha ao salvar o evento.",
      )
    } finally {
      setSaving(false)
    }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setFormError(
        error instanceof Error ? error.message : "Falha ao excluir o evento.",
      )
    } finally {
      setDeleting(false)
    }
  ...
  const availableMembers = React.useMemo(
    () =>
      availableMembersByWorkspaceId[eventWorkspaceId] ??
      availableMembersByWorkspaceId[selectedWorkspaceId] ??
      [],
  ...
  React.useEffect(() => {
    ...
    const targetEvent = data
      .flatMap((entry) => entry.events)
      .find((event) => event.id === focusEventId)
      ...
      <div className="mt-2 flex items-center justify-end">
        {assignees.slice(0, maxVisible).map((assignee, index) => (
          <Avatar
            ...
            title={assignee.name}
            className={cn(
              "h-6 w-6 border border-white/10 bg-black/60",
              index > 0 && "-ml-2",
            )}
          >
            {assignee.imageSrc ? (
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {format(endOfMonth(visibleMonth), "dd MMM yyyy", {
                  locale: ptBR,
                })}
              </p>
            </div>
            ...
            <div className="flex items-center rounded-md border border-white/15 bg-black/40">
              <Input
                ...
                onChange={(event) =>
                  setSelectedDate((current) =>
                    moveDateToMonth(current, event.target.value),
                  )
                }
              ...
              <div className="mr-1 flex flex-col gap-1">
                <button
                  type="button"
                  onClick={() =>
                    setSelectedDate((current) => shiftDateByMonths(current, 1))
                  }
                  ...
                  [
                    {
                      ...
                      items: groupedEvents.past,
                    },
                    {
                      key: "today",
                      title: "Eventos de hoje",
                      items: groupedEvents.today,
                    },
                    {
                      key: "future",
                      title: "Eventos futuros",
                      items: groupedEvents.future,
                    },
                  ] as const
                ).map((section) => (
            ...
            <SheetHeader>
              <div className="flex items-start justify-between gap-3">
                <div className="space-y-1">
                  ...
                  <SheetDescription>
                    Preencha por etapas até concluir o salvamento.
                  </SheetDescription>
                </div>
                    ...
                    <>
                      ...
                      <div className="space-y-2">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setEventWorkspaceId(event.target.value)
                          }
                      ...
                      <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <button
                            ...
                            onClick={() =>
                              setEventIsMeeting((current) => !current)
                            }
                            className={cn(
                        ...
                        {eventIsMeeting ? (
                          <div className="mt-3 space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(event) =>
                                setEventMeetingLink(event.target.value)
                              }
                      ...
                      <div className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-foreground">
                        ...
                        {eventAssigneeIds.length > 0 && (
                          <p className="mt-2 text-xs text-muted-foreground">
                            {eventAssigneeIds.length} participante
                            {eventAssigneeIds.length > 1 ? "s" : ""} atribuído
                ...
                <div className="flex items-center gap-3">
                  ...
                  {currentStep < 2 ? (
                    <Button
                      ...
                      disabled={
                        !!validateStep(currentStep) || saving || deleting
                      }
                    >
                      Próximo
                    </Button>
