...
function logRpcFailure(name: string, error: string) {
  console.warn(`[calendar-notifications] ${name} falhou (aplica supabase/calendar_event_assignees.sql se necessario):`, error)
}
...
export async function rpcNotifyCalendarEventAssignees(args: {
  ...
  const { data, error } = await supabase.rpc("notify_calendar_event_assignees_in_app", {
